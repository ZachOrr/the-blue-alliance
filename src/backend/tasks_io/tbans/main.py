...
@staticmethod
def ping(client: MobileClient):
    """ Immediately dispatch a Ping to either FCM or a webhook """
    if client.client_type == ClientType.WEBHOOK:
        return _ping_webhook(client)
    else:
        return _ping_client(client)

@staticmethod
def _ping_client(client: MobileClient):
...
@staticmethod
def _ping_webhook(client: MobileClient):
    from models.notifications.ping import PingNotification
    notification = PingNotification()
